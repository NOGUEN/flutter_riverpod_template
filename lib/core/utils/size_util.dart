...
class SizeUtil{
  ...
  /// 높이 값 얻기
  static double heightDouble(double height){
    return deviceSize.height * (height/812);
  }

  /// 넓이 값 얻기
  static double widthDouble(double width){
    return deviceSize.width * (width/375);
  }

}

extension SizeUtilExtensionOnInt on int {
  double get w => SizeUtil.width(this);
  double get h => SizeUtil.height(this);
}

extension SizeUtilExtensionOnDouble on double {
  double get w => SizeUtil.widthDouble(this);
  double get h => SizeUtil.heightDouble(this);
}
